...
{
    try{
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/order/buyer/orders`,
        {
            headers: {
                Authorization : `Bearer ${token}`
            }
...
export const CancelOrder = async (token, orderId, handleAlert, navigate) => {
    try{
        const response = await axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/order/cancel`,
        {
            undefined,
            orderId
        },
...
export const GetOrderDetails = async (handleAlert,token, orderId, userType) =>
{
    ...
    {
        try{ 
            const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/order/seller/details`,
            {
                params: { 
                    orderId
                },
        ...
        try{ 
            const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/order/details`,
            {
                params: { 
                    orderId
                },
...
export const GetSellersNewOrders =  async (handleAlert, token) => {
    try{
        const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/Order/seller/new-orders`,
        {
            headers: {
                Authorization : `Bearer ${token}`
            }
